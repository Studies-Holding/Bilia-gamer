...
Cœur de l'expérience de jeu (Gaming Platform) : **parties, salles, sauvegardes**, **moteur de règles métier**, **égaliseur de niveau**, **matchmaking**, succès, classements, historique. C'est la vérité métier d'une session ; le temps réel est relayé par `realtime-service`.
...
- **MVP :** créer/rejoindre/quitter/reprendre une partie, salles privées (code), sauvegardes, **moteur de règles v0** (tours, chronomètre, score, manches, équipes, conditions de victoire), **égaliseur de niveau** (attribution d'un niveau par profil), **matchmaking v0** (appariement par niveau/égaliseur pour les parties publiques, cf. AFG-004 SDK Matchmaking, AFG-DT-003 Lot 3), succès et classements de base.
- **Phase 2 :** tournois (délégué à `tournament`), classements avancés, défis, égaliseur dynamique, règles étendues (éliminations, quêtes, saisons, campagnes, arbitrages auto), **matchmaking étendu** (âge, langue, pays, compétences IDC, contexte familial : AFG-004 ch.28).
...
- `MatchmakingTicket` : demande d'appariement (profil, critères : niveau/âge/langue/pays/IDC : statut).
...
- `POST /matchmaking/tickets`, `GET /matchmaking/tickets/:id`, `DELETE /matchmaking/tickets/:id` (annulation)
...
- **Produits :** `SessionStarted`, `SessionEnded` (avec compétences mobilisées), `MatchFound`, `AchievementUnlocked`, `ScoreRecorded`.
- **Consommés :** `CurfewChanged` (identity → coupure/limitation), `TournamentMatchScheduled` (tournament).

## 6. Dépendances

- **S :** `identity` (profils + couvre-feu), `catalog` (droit d'accès + config jeu), `payment` (achats in-game), `wallet` (crédit direct de récompenses en Jetons : succès/fin de partie, P2, sans passer par `payment`), `griot` (contexte culturel), `skills-idc` (mapping compétences), `ai` (égaliseur piloté par IA : P3).
...
│   │   ├── matchmaking.service.ts     # appariement par critères (niveau, âge, langue, pays, IDC)
│   │   └── scoring.service.ts
...
5. **Matchmaking v0** (`MatchmakingTicket`, appariement par niveau/égaliseur pour parties publiques) + tests.
6. Succès + classements + historique ; `SessionEnded` → compétences.
7. Application du couvre-feu au runtime (consumer `CurfewChanged`).
8. Runtime action v0 : game server stateful + boucle de tick + NullEngine + physique Havok (1re expérience action).
9. (P2) Règles étendues + défis + égaliseur dynamique + hooks tournois ; **matchmaking étendu** (âge, langue, pays, IDC, contexte familial) ; option Rapier déterministe pour les modes compétitifs.
10. (P3) Cross-plateforme, égaliseur IA, cloud gaming.
