...
- `game-service` : parties, **salles privées (code)**, sauvegardes, **moteur de règles métier (v0 : tours, score, manches, conditions de victoire)**, **égaliseur de niveau (par profil)**, succès, classements, **matchmaking v0 (appariement par niveau/égaliseur, cf. AFG-004 SDK Matchmaking)**.
...
- `wallet-service` : **Wallet** (solde), **Jetons (BiCoins)**, grand livre : seul service qui écrit sur le solde ; exposé en interne (S) à `payment`, `game`, `tournament`. Cf. AFG-DT-001 ADR-09.
- `payment-service` : **Mobile Money** (intégration PSP), cartes, **abonnements Solo/Famille**, achats idempotents, répartition revenus créateurs (v0). Débite/crédite le Wallet via appel synchrone à `wallet-service` : ne touche jamais le solde directement.
- Flux achat bout-en-bout : catalog → payment → wallet (si Jetons) → droit d'accès.
**Dépend de :** Lot 2.
